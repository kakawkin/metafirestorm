...
function StatsSourceModalContent(){
  return (
    <React.Fragment>
      ...
      <StatsInfoBlock icon="🙆" title="Не соответствие данных вторичных стат">
        После сжатия стат на миднайте стало невозможным получить адекватные вторичные статы предметов - они чуть завышены.<br/>
		Суммарная разность вторичных стат может отклоняться на ~3000
      </StatsInfoBlock>
      ...
      <StatsInfoBlock icon="🎒" title="Про вещи">
        ...
        <br/>Это <strong style={{color:'var(--danger)'}}>не BIS-лист</strong> — BIS лист предполагает что у вас есть абсолютно любые вещи на выбор
      ...
      <StatsInfoBlock icon="📈" title="Про статы">
		...
		<br/><br/>
		<strong style={{color:'var(--accent)'}}>Главное правило:</strong> В 99% случаев мейнстат всегда важнее каких-то вторичек. Исключение это когда есть вещь 707 уровня с двумя хорошими вторичками, и есть 710 с двумя самыми ненужными вторичками - тут уже только симить.
		<br/><br/>
		...
		Больше основание с меньшим показателем даёт больший результат нежели наоборот. Зачастую искусь апает только часть абилок - и вы добавляете 5 абилкам 2% урона, теряя с 13 абилок по 0.5% 
		<br/>
		Понизили урон 20 спелкам на 3% что-бы дать 3-4 абилкам 5%. <br/><br/>
		Яркий пример БМ охотник в гер ветке пак лидер - дамажат петы, а искусь собирают меньше всего которая их должна апать.
      </StatsInfoBlock>
    </React.Fragment>
  );
}
